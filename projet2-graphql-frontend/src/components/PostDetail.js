...
export default function PostDetail() {
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white p-6 pr-10 pl-10 mt-8">
      <div className="mb-8 border-b pb-4">
        ...
        <div className='flex flex-wrap'>
          <p className="text-gray-600 mr-4 underline">{postData.post.author}</p>
          <p className="text-gray-500">{formatDate(postData.post.createdAt)}</p>
          <p className="text-gray-500 ml-4 underline">
            {commentsData.comments.length} comment{commentsData.comments.length !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mb-4 border-b pb-4">
        ...
        <button type="submit" className="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-8 rounded-full transition duration-300">
          Add Comment
        </button>
      </form>
